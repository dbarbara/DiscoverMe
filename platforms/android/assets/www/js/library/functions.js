...
/*class for showing upload progress to a user:*/
var upload_progress_bar = {
	current_state:0,
	is_open:false,
	init: function() {
		/*only initialize if not already open*/
		if($('.progress_bar.full.atopleft.overlay').length == 0){
			this.current_state = 0;
			/*create window...*/
			$('body').append(
				'<div class="progress_bar full atopleft overlay">'
					+ '<div class="progress_loader" style="background:#080808;6px solid #635f60;border-radius:10px;width: 60%;height: auto;position: absolute;top: 25%;left: 20%;border: 6px solid #635f60;padding: 10px;margin-left: -13px;">'
						+ '<div class="title m_m">Upload in Progress...</div>'
						+ '<div class="title s_m">Status: <span class="status"></span></div>'
						+ '<div class="progress m_m"><div stlye="width:0%;"></div></div>'
					+ '</div>'
				+ '</div>'
			);
			this.update('Uploading...',0);
			this.is_open = true;
		}
	},
	update: function(state,amount){
		$('.progress_loader .status').text(state);
		$('.progress_loader .progress div').css('width',(amount * 100) + '%');
	},
	destroy: function(success){
		console.log("destroying the progress bar...");
		$('.progress_bar.full.atopleft.overlay').remove();
		this.current_state = 0;
		if(!success){
			SNApp.showDialog('You need to check into this listing before you can add notes to it.');
		}
		this.is_open = false;
	}
}